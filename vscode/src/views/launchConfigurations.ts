...
export function updateLaunchConfig() {
    workspace.findFiles('.vscode/launch.json').then(async files => {
        ...
        for (const file of files) {
            ...
            const updateOption = l10n.value("jdk.extension.runConfig.label.updateExistingLaunchJson");
            if (newContent !== content) {
                if (!selection) {
                    selection = await window.showWarningMessage(l10n.value("jdk.extension.runConfig.warning_message.renamedDebugConfig"), updateOption);
                }
