...
import { debugVar, registerDebugCommands, registerDebugger } from './debugger/debugger';
...
export function contextUri(ctx : any) : vscode.Uri | undefined {
    if (ctx?.fsPath) {
            ...
            if (!srv) {
                ...
                srv.stdout.on("data", (chunk) => {
                    if (debugVar.port < 0) {
                        ...
                        if (info) {
                            debugVar.port = info[1];
                            debugVar.hash = info[2];
                        }
                    }
                    if (!lspServerStarted) {
